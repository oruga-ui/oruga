...
| scrollStrategy    | Strategy used to prevent background scrolling when open.<br/>`clip` removes the body scrollbar via `overflow: hidden`.<br/>`keep` maintains a phantom scrollbar via `position: fixed` to prevent layout shift. | "clip" \| "keep"                     | `clip`, `keep`                                                                                                         | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>dropdown: {<br>&nbsp;&nbsp;scrollStrategy: "keep"<br>}</code>      |
