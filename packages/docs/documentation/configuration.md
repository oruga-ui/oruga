...
And **all components come with predefined classes** by default, which you can use to create your own styling.
...
The easiest way to customise your components is to set up a global configuration object with a <i>class-mapping</i> for each component.
...
::: info
While using TypeScript, the configuration object is defined by the type `OrugaConfig`.
:::
...
All `class` properties for each component can be found in the `Class Inspector` section of the respective component page.  
If it turns out that you also have classes defined globally in the config for this component, they will be merged together by default.
...
| globalComponents   | Specify whether all components should be registered globally. By default, no components will be registered globally.                                                                         | <code style='white-space: nowrap; padding: 0;'>false</code>                                                                                                                                                       |
