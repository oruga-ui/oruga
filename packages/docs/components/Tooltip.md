...
| <s>triggers</s>   | <div><b>deprecated</b> - will be removed - use `triggerOnClick`, `triggerOnHover`, `triggerOnContextmenu` and `triggerOnFocus, instead</div><div><s>Tooltip trigger events</s></div>                  | ("focus" \| "click" \| "hover" \| "contextmenu")[]                                                               | `hover`, `click`, `focus`, `contextmenu`                                                         | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>tooltip: {<br>&nbsp;&nbsp;triggers: []<br>}</code>             |
