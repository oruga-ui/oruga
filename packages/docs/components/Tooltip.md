...
| <s>closeable</s>  | <div><b>deprecated</b> - will be only boolean - use `closeOnOutside` and `closeOnEscape` instead</div><div><s>Tooltip auto close options (pressing escape, clicking the content or outside)</s></div> | boolean \| ("content" \| "outside" \| "escape")[]                                                                | `true`, `false`, `content`, `outside`, `escape`                                                  | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>tooltip: {<br>&nbsp;&nbsp;closeable: true<br>}</code>          |
