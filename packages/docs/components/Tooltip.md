...
| <s>closeable</s> | <div>Deprecated: will be only boolean - use `closeOnOutside` and `closeOnEscape` instead</div><hr/><div><s>Tooltip auto close options (pressing escape, clicking the content or outside)</s></div> | boolean \| ("content" \| "escape" \| "outside")[]                                                                | `true`, `false`, `content`, `outside`, `escape`                                                  | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>tooltip: {<br>&nbsp;&nbsp;closeable: true<br>}</code>             |
