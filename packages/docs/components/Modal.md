...
| <s>cancelable</s> | <div><s>Is Modal cancelable by clicking 'X', pressing escape or clicking outside</s></div><hr /><div><b>deprecated</b> - use `closeable`, `closeOnOutside` and `closeOnEscape`</div>                                                                   | boolean \| string[]         | `escape`, `x`, `outside`, `button`, `true`, `false` | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>modal: {<br>&nbsp;&nbsp;cancelable: undefined<br>}</code>       |
