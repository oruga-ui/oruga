...
| <s> cancelable </s> | Is Sidebar cancelable by pressing escape or clicking outside. - use `closeOnOutside` and `closeOnEscape`                                                                                                        | boolean \| string[]                    | `escape`, `outside`, `true`, `false` | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>sidebar: {<br>&nbsp;&nbsp;cancelable: undefined<br>}</code>       |
