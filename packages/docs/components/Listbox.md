...
| filter      | Overridet the filter input                | **value** `string` - filter input value<br/>**on-change** `(input: string, event: Event): void` - on filter input change event<br/>**on-keydown** `(event: Event): void` - on filter input keydown event |
