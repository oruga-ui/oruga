...
| animation         | Transition animation name                                                                                                     | Array&lt;string&gt; | `[next`, `prev]`, `[right`, `left`, `down`, `up]`                               | <div><small>From <b>config</b>:</small></div><code style='white-space: nowrap; padding: 0;'>steps: {<br>&nbsp;&nbsp;animation: [ "slide-next", "slide-prev", "slide-down", "slide-up", ]<br>}</code> |
...
| change            | **value** `string \| number` - new step value<br/>**value** `string \| number` - old step value | on step change event            |
