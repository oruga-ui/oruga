    ...
    computed: {
        ...
        translation() {
            return -bound(
                ...
                0,
                (this.childItems.length - this.settings.itemsToShow) *
                    this.itemWidth,
            );
        },
        total() {
        ...
        hasPrev() {
            return (
                (this.settings.repeat || this.scrollIndex > 0) && this.hasArrows
            );
        },
        hasNext() {
            return (
    ...
    methods: {
        ...
        startTimer() {
            ...
            this.timer = setInterval(() => {
                if (
                    !this.repeat &&
                    this.activeIndex >= this.childItems.length - 1
                ) {
                    this.pauseTimer();
                } else {
                    this.next();
        ...
        switchTo(newIndex, onlyMove = this.asIndicator) {
            ...
            if (!onlyMove) {
                this.activeIndex = Math.ceil(
                    newIndex / this.settings.itemsToList,
                );
